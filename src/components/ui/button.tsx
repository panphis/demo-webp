...
export const Button = ({ className, variant, size, children, disabled, ...props }: ButtonProps) => {
  return (
    <button
      data-slot="button"
      disabled={disabled}
      className={cn(buttonVariants({ variant, size, className }), disabled && "opacity-50 hover:cursor-not-allowed")}
      {...props}
    >
      {children}
    </button>
